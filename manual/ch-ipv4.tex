...
% FIXME (#459) there is no fragmentation if the packet is delivered through a local interface (e.g. loopback).
...
% FIXME: (#461) TTL<=0 should be checked in fragmentAndSend() before fragmenting. (sendDatagramToOutput only called from fragmentAndSend())
...
% FIXME (#458) local broadcasts from HL does not looped back, it is necessary because network cards do not receive the messages they send
% FIXME (#457) when a broadcast message is locally delivered and its source address is unspecified,
...
% FIXME (#469) why the global cache is cleared when an ARP module is deleted?
...
\begin{compactitem}
 \item \ttt{@hosts} specifies a list of host name patterns
 ...
 \item \ttt{@address} IP address of the interface (e.g. "192.168.1.1" or
   "10.0.x.x").
...
\begin{compactitem}
  ...
  \item \ttt{@interfaces} specifies a list of interface name patterns.
     Only interfaces with the specified names are affected. The
     default value is "*" that matches all interfaces.

  \item \ttt{@address}: one or more multicast address, that are added
...
\begin{compactitem}
 ...
 \item \ttt{@destination} specifies the destination address of the route.
...
\begin{compactitem}
 ...
 \item \ttt{@source} specifies the address of the source network.
  The default value is "*" that matches all sources.

 \item \ttt{@netmask} specifies the netmask of the source network.
  The default value is "*" that matches all sources.

 \item \ttt{@groups} list of IPv4 multicast addresses specifying the groups
   this entry applies to (e.g. "225.0.0.1 225.0.1.2").
   The default value is "*" that matches all multicast groups.

 \item \ttt{@metric} specifies the metric in the route.

 \item \ttt{@parent} specifies the name of the parent (incoming) interface.
 ...
 \item \ttt{@children} specifies the child (outgoing) interfaces as a list of interface name
   patterns:
   \begin{compactitem}
...
% FIXME (#465) shortest path calculation should exclude the nodes where IPForward is disabled.
...
% FIXME (#468) packetLength declared as volatile, but in fact it is not.
